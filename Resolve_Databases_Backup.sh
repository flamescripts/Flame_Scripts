...
# Author not responsible for any data loss from use.  Do not use
# unless you understand every command and it's risk.
# Data loss possible.
...
do
	...
	DBbacknum=$(ls -1t $RSYNCpath | grep $DBname | grep $DBip | wc -l)

	#Tally processes and report current task
	echo -e "Processing Job $DBtaskcurrent of $DBtasktotal: Checking Database: ${DBname} on IP: ${DBip}"

	# This should have tighter conditionals.
  #	if [ $(ping -c 2 -W 1 ${DBip} | grep 'received' | awk -F',' '{ print $2 }' | awk '{ print $1 }') -eq 2 ];
	...
	then
		...
		# If 4 versions exits of backup, remove oldest backup keep disk usage in-check
		if [ "$DBbacknum" -gt 3 ];
		then
			echo -e "Rollover: \033[33mRemoving oldest backup of ${DBip}:${DBname} > `ls -1t $BUdatabases | grep Resolve_${DBname}_${DBip}_ | tail -n 1` "
			## use below rm at your own risk.  Comment out until you test .
      ls -1t | grep Resolve_${DBname}_${DBip}_ | tail -n 1 | xargs rm -f
			ls -1t $BUdatabases | grep Resolve_${DBname}_${DBip}_ | tail -n 1
		else
			echo -e "Rollover: Unable to remove oldest backup of ${DBip}:${DBname} > Not enough backups available, skipping removal"
		fi
		echo
	else
...
# Check if network share is available, if available rsync local backup to share
	echo -e "Processing Job $DBtasktotal of $DBtasktotal: Checking Network Share Availability and Backing up the Backups"
